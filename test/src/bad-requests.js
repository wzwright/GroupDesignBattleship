...
let step = 0
function next() {
  console.log('starting tests for bad requests...')
...
actions.push(() => {
  api.doRPC('newGame', ['too many', 'arguments?'],
    ([gameID, playerID]) => {
      console.error('Game was created with invalid params')
    },
    (e) => {
      next()
    },
...
actions.push(() => {
  api.joinGame(
    ...
    'p2',
    (playerID) => {
      console.error('Third player was allowed to join game')
    },
    (e) => {
      next()
    },
...
actions.push(() => {
  api.submitGrid(
    player1,
  [[2, 1, 6, 1], [0, 2, 3, 2], [1, 3, 3, 3], [2, 4, 4, 4]],
    () => { console.error('Accepted bad grid') },
    (e) => { next() },
)
})
actions.push(() => {
...
actions.push(() => {
  api.bombPosition(
    ...
    0,
    (hit) => {
      console.error('Allowed bombing same location twice')
      next(0)
    },
    (e) => {
      next()
...
actions.push(() => {
  api.bombPosition(
    ...
    7,
    (hit) => {
      console.error('Allowed bombing after end of game')
    },
    (e) => {
      next()
    },
